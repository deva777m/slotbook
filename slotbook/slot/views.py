        ...
        if user.email1 == email or user.email2 == email:
            if "CLASH" in event_list or "RC" in event_list or "ENIGMA" in event_list:
                if user.session == "true":
                    return render(request, 'slot.html', {'user': user, 'events': event_list})
...
def book(request):
    if request.method == 'POST':
        ...
        if event == "#" or day == "#" or time == "#":
            alert = "Please Select the Options"
            return render(request, 'slot.html', context={'events': events, 'user': user, 'msg': alert})
        ...
        if event == "CLASH":
            ...
            dict = SlotsTiming.objects.filter(event='CLASH')
            ...
            if day == 'FRIDAY':
                dict = SlotsTiming.objects.filter(event='ENIGMA', day='FRIDAY')
            elif day == 'SATURDAY':
                dict = SlotsTiming.objects.filter(event='ENIGMA', day='STURDAY')
...
def change_drop(request):
    ...
    if event == "CLASH":
        dict_t = SlotsTiming.objects.filter(event='CLASH')
        ...
        if day == 'FRIDAY':
            dict_t = SlotsTiming.objects.filter(event='ENIGMA', day='FRIDAY')
        elif day == 'SATURDAY':
            dict_t = SlotsTiming.objects.filter(event='ENIGMA', day='SATURDAY')

    for d in dict_t:
        list_t.append(d.time_slot)

    print(list_t)
    return JsonResponse({"slots": list_t})
